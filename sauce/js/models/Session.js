...
const Session = {
  ...
  _storage: JSON.parse(window.localStorage.getItem('data')) || {},
  _getStoragePath: (key = '', create_path = false) => {
    if (key === '') return { obj: Session._storage, element: '' }
    const hierarchy = key.match(/([^\\\.]|\\.)+/g)
    let last_obj = null
    let last_name = ''
  ...
  _set: (key, value, is_default = false) => {
    ...
    const { obj, element } = Session._getStoragePath(key, true)
    if (element === '') {
  ...
  _get: (key = '') => {
    const { obj, element } = Session._getStoragePath(key, false)
    if (obj == null || element == null) return null
    return obj[element]
  }
}
window.addEventListener('unload', () => {
  window.localStorage.setItem('data', JSON.stringify(Session._storage))
})
...
const Playlist = {
  ...
  insert: (base_path, files, pos = Playlist.items.length) => {
    ...
    m.request({
      method: 'GET',
      url: `${Config.serverAddress}/api/info${encodeURIComponent(base_path)}`,
  ...
  get: (index = Playlist.current_index) => {
  ...
  isSelected: index => {
    return !!Playlist.selected_items[index]
  },
  isChecked: index => {
    return !!Playlist.checked_items[index]
  },
  toggleChecked: index => {
    if (index < 0 || index >= Playlist.items.length) return
    Playlist.checked_items[index] = Playlist.checked_items[index] !== true
  },
  check: index => {
    if (index < 0 || index >= Playlist.items.length) return
